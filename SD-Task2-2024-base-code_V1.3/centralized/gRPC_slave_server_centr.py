...
class SlaveServiceServicer(store_pb2_grpc.KeyValueStoreServicer):
    ...
    def put(self, request: store_pb2.PutRequest, context: grpc.aio.ServicerContext) -> store_pb2.PutResponse:
        ...
        self.semaphore.release()
        return store_pb2.PutResponse(accept=True)
    ...
    def slow_down(self, request: store_pb2.SlowDownRequest, context: grpc.aio.ServicerContext) -> store_pb2.SlowDownResponse:
        ...
        return store_pb2.SlowDownResponse(accept=True)
    ...
    def can_commit(self, request: store_pb2, context: grpc.aio.ServicerContext) -> store_pb2.CanCommitResponse:
        return store_pb2.CanCommitResponse(accept=True) # Always return True in order to accept request modification
    ...
    def restore(self, request: store_pb2.RestoreRequest, context: grpc.aio.ServicerContext) -> store_pb2.RestoreResponse:
        ...
        return store_pb2.RestoreResponse(accept=True)

def slave_server(port_node):
    server = grpc.aio.server(futures.ThreadPoolExecutor(max_workers=10)) # Create a gRPC server with 10 threads
    ...
    print(f"[!] Starting Master Server. Listening on port {port_node}") # Print message to show that the server is running
    server.add_insecure_port(f"localhost:{port_node}") # Set the port for the server, specified in documentation
